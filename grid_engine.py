"""Grid trading engine: spacing, placement, fills, ratchet, session end."""
...
class GridEngine:
    ...
    def place_buy(self, price: int, *, tag: str = "", ratchet: bool = False, recycle: bool = False) -> Optional[Order]:
        if not recycle and not ratchet and not self.can_place_new_buy():
            self._log("engine.skip_buy", f"limit: holdings={self.holdings_qty()} new_buys={self.grid.new_buys_filled_today if self.grid else 0}", {})
            return None
    ...
    def place_initial_grid(self) -> list[Order]:
        ...
        for i, px in enumerate(self.grid.buy_lines):
            ...
            if open_buys >= remaining_slots and remaining_slots >= 0:
                ...
                pass
            o = self.place_buy(px, tag=f"grid_L{i+1}@{px}")
            if o:
                placed.append(o)
    ...
    def on_session_start(self, ref_price: int, now: datetime) -> None:
        ...
        self._log(
            ...
            {"date": str(self._today), "holdings": len(self.positions)},
        )
        self.recalc_grid(ref_price)
        # Restore sells for existing positions
        for p in self.positions:
    ...
    def _handle_fill(self, order: Order, now: datetime) -> None:
        ...
        if order.side == Side.BUY:
            ...
            if removed is not None:
                px = removed.grid_line or removed.buy_price
                self.place_buy(
                    px,
                    tag=f"recycle@{px}",
                    recycle=True,
